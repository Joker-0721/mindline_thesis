...
本章阐明剪切自锁现象的成因与抑制途径，为后续混合离散格式的构造提供理论依据。首先从 Mindlin 板理论的位移场与应变-位移关系出发，分析薄板极限下剪切应变约束导致的数值刚化机制；其次归纳传统有限元中减缩积分与位移-剪力混合离散两类免自锁方案的原理与局限；随后引入约束比与 LBB 条件，说明最优约束比的选取原则；最后给出本文采用的无网格增强免自锁方案的基本思想与 Mindlin 板运动学方程。
